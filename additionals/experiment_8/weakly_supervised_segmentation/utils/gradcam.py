...
# This file was adapted for experiment 8. More details can be found below in the code.
...
# Experment 8 : Generate boxes from CAM contours, train a box‑guided segmentation and compare.
# Code adapted from https://github.com/ShichengChen/TRStrain/blob/199130eb65ba98e2dd6cbc4d8b740b7d39492478/main.py 
def box_extraction(cam):
    cam_np = cam.detach().cpu().numpy()[0, 0]
    thresh= 0.5
    contours, _ = cv2.findContours((cam_np> thresh).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    if not contours:
        h,w = cam_np.shape
        return [0,0,w,h]
    x_min = float('inf') 
...
# Experment 8 : Generate boxes from CAM contours, train a box‑guided segmentation and compare.
# This function is also adapted for experiment 8
def generate_pseudo_masks(classifier, dataloader, device, threshold=0.5, save_visualizations=True, 
                         visualization_dir="gradcam_visualizations", box_tracking = True) :
    """
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader, desc="Generating pseudo-masks")):
        ...
        for i in range(images.size(0)):
            ...
            rectangle = None
            if box_tracking == True:
                rectangle= box_extraction(cam)

                h,w = cam.shape[2:]
                mask_rectangle= torch.zeros((1,1,h,w), device=cam.device)
                x_min, y_min, x_max, y_max = rectangle
                mask_rectangle[0,0,y_min:y_max,x_min:x_max]=1
                cam = cam * mask_rectangle
            
            # Initial adaptive percentile thresholding
            pseudo_mask = percentile_threshold(cam, percentile=80)

            # Safety check for extremely sparse masks
            ...
            # Final fallback (absolute safety)
            ...
            if save_visualizations:
                ...
                if box_tracking and rectangle is not None:
                    img_part = img.detach().cpu().numpy()[0].transpose(1, 2, 0)
                    img_part = (img_part - img_part.min()) / (img_part.max() - img_part.min())
                    ...
                    plt.title("Constraints on CAM ")
                    plt.axis('off')
                    
                    plt.subplot(1, 3, 3)
                    plt.imshow(pseudo_mask[0,0].cpu().numpy(), cmap='gray')
                    plt.title("Box Mask")
                    plt.axis('off')
                    ...
                    save_gradcam_visualization(img, cam, pseudo_mask, save_path)
            ...
            if box_tracking and rectangle is not None:
                original_image["rectangle"] = rectangle
            
            pseudo_labeled_data.append(original_image)
