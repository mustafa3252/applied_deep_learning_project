...
def generate_pseudo_masks(classifier, dataloader, device, threshold=0.5, save_visualizations=False, 
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader, desc="Generating pseudo-masks")):
        ...
        for i in range(images.size(0)):
            ...
            # First CAM (original image)
            layer4_cam1 = layer4_gradcam(img, true_class)[0]
            layer3_cam1 = layer3_gradcam(img, true_class)[0]
            cam1 = layer4_cam1 * 0.7 + layer3_cam1 * 0.3

            # Second CAM (erasing)
            # Erase top activated region
            ...
            cam2 = layer4_cam2 * 0.7 + layer3_cam2 * 0.3

            # Third CAM (flipped image)
            ...
            cam3 = horizontal_flip(cam3)  # Flip CAM back to original orientation

            cam = torch.max(torch.max(cam1, cam2), cam3)

            # Threshold and refine
            pseudo_mask = percentile_threshold(cam, percentile=80)
            min_foreground_pixels = 0.01 * pseudo_mask.numel()
