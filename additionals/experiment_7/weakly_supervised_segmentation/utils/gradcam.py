...
class GradCAM:
    ...
    def __call__(self, input_tensor, target_class=None):
        ...
        if isinstance(output, dict) and 'out' in output:  # Segmentation model
            output = output['out']
...
def generate_pseudo_masks(classifier, dataloader, device, threshold=0.5, save_visualizations=False, 
    ...
    layer4_gradcam = GradCAM(classifier, classifier.encoder.layer4[-1])
    layer3_gradcam = GradCAM(classifier, classifier.encoder.layer3[-1])
