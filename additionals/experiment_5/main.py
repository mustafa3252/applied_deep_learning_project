...
def main():
    ...
    for learning_rate_value, weight_decay_value in product(learning_rate, weight_decay):
        ...
        if current_accuracy > overall_best_acc:
            ...
            best_parameter_pairs = {"learning_rate": learning_rate, "weight_decay": weight_decay}
    ...
    for learning_rate_segmentation_value, weight_decay_segmentation_value in product(learning_rate_segmentation,
                                                                                     weight_decay_segmentation):

        currect_iou, current_state_seg, binary_metrics, track_train_seg_loss, track_iou_seg = segmentation_hypertuning_training(
            pseudo_loader, test_loader_seg, device, learning_rate_segmentation_value, weight_decay_segmentation_value,
            epochs=4
        )
        ...
        if currect_iou > overall_best_segmentation_iou:
            ...
            best_parameter_pairs_segmentation = {"learning_rate_segmentation": learning_rate_segmentation,
    ...
    # Step 4.5: Fine-tune on real 10% masks
    log_message("\n4.5 Fine-tuning on real masks...")
    # Load the full train dataset with real masks
    full_dataset = OxfordPetSegmentation(root='./data', split='trainval',
                                         transform_img=transforms['train'],
                                         transform_mask=transforms['mask'])
    segmentation_model = fine_tune_on_real_masks(segmentation_model, full_dataset, test_loader_seg, device, epochs=5)
