...
def percentile_threshold(cam, percentile=40):
...
def flip_mask_pixels(mask, noise_ratio):
    # Flip `noise_ratio` fraction of pixels in a binary mask.
    m = mask.detach().cpu().numpy()[0,0].astype(np.uint8)
    total = m.size
    k = int(noise_ratio * total)
    idx = np.random.choice(total, k, replace=False)
    flat = m.flatten()
    flat[idx] = 1 - flat[idx]
    m_noisy = flat.reshape(m.shape)
    t = torch.from_numpy(m_noisy).float()
    return t.unsqueeze(0).unsqueeze(0).to(mask.device)
