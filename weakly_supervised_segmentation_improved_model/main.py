...
def main():
    ...
    # Replace single values with multiple values for a proper grid search
    ...
    for learning_rate_value, weight_decay_value in product(learning_rate, weight_decay):
        ...
        current_accuracy, current_classifier_state = classifier_training_hypertuning(
            train_loader_cls, val_loader_cls, device, learning_rate_value, weight_decay_value, epochs=4
        )
        ...
        if current_accuracy > overall_best_acc:
            ...
            best_parameter_pairs = { "learning_rate": learning_rate_value, "weight_decay" : weight_decay_value}
    ...
    # STEP 3.1: only 20% synthetic noise (best‐performing)
    noise_levels = [0.3]
    results = {}
    for nl in noise_levels:
        log_message(f"\nTraining with {int(nl*100)}% synthetic noise")
        
        # Create dataset with pseudo-labels and noise
        ...
        for epoch in range(5):  # Can adjust number of epochs
            train_loss = train_segmentation(segmentation_model, loader, optimizer, criterion, device)
            metrics = evaluate_segmentation(segmentation_model, test_loader_seg, binary_metrics, device)
            log_message(f"Epoch {epoch+1}, Loss: {train_loss:.4f}, IoU: {metrics['IoU']:.4f}")
            best_iou = max(best_iou, metrics['IoU'])
        results[nl] = best_iou

    # Report 20% noise result
    log_message("\n30 % Noise | Best Val IoU")
    log_message("------------------------")
    log_message(f"   30 %     |   {results[0.3]:.4f}")

    # Visualize some pseudo-masks
    # (we just trained on `dataset`, so alias it here for clarity)
    visualize_pseudo_masks(dataset, output_dir="gradcam_visualizations")
    
    log_message("\n4. Training segmentation model on pseudo-masks (20% noise)…")
    log_message("Grid search on segmentation hyperparameters using the 20% noise loader")
    seg_lrs = [0.0001]
    seg_wds = [0.00001]       # Try different weight decay values
    overall_best_segmentation_iou = 0.0
    overall_best_segmentation_state = None
    best_parameter_pairs_segmentation = {}
    tracking_seg = {}
    
    # NOTE: use the `loader` you just created for 20% noise
    seg_train_loader = loader
    
    for lr_val, wd_val in product(seg_lrs, seg_wds):
        val_iou, seg_state, binary_metrics, track_train_seg_loss, track_iou_seg = \
            segmentation_hypertuning_training(
                seg_train_loader,
                test_loader_seg,
                device,
        ...
        parameter_pairs = f"learning_rate={lr_val}, weight_decay={wd_val}"
        ...
        if val_iou > overall_best_segmentation_iou:
            ...
            best_parameter_pairs_segmentation = {"learning_rate_segmentation" : lr_val, "weight_decay_segmentation" : wd_val}
